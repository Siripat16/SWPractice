...
export default function UserProfileScreen({ navigation }) {
  ...
  async function fetchUserProfile(userID) {
    try {
      const url = `http://172.20.10.2:8081/api/v1/auth/user/${userID}`;
      // const url = `http://127.0.0.1:2000/api/v1/auth/user/${userID}`;
      const response = await axios.get(url);
      if (response.data.success && response.data.data) {
        const profileData = response.data.data;
        const imageUri = profileData.picture
  ...
  async function uploadImage() {
    ...
    try {
      const response = await axios({
        ...
        url: `http://172.20.10.2:8081/api/v1/auth/user/${studentID}`,
        method: "PUT",
        data: formData,
        headers: {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const url = `http://172.20.10.2:8081/api/v1/auth/user/${studentID}`;
